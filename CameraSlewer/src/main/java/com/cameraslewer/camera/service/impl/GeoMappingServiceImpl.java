package com.cameraslewer.camera.service.impl;
...
import com.cameraslewer.camera.repositories.CameraRepository;
import com.cameraslewer.camera.repositories.LocationRepository;
import com.cameraslewer.camera.service.GeomappingService;
import com.cameraslewer.camera.validators.RequestValidator;
...
public class GeoMappingServiceImpl implements GeomappingService {
	...
	public GetNearByLocationsResponse getNearByLocation(GetNearByLocationsRequest request) {
		...
		List<LocationEntity> locationList = locationRepository.findByLocationNear(
				new Point(Double.valueOf(camera.getLongitude()), Double.valueOf(camera.getLatitude())));
		if (locationList != null && !locationList.isEmpty()) {
	...
	private CameraEntity getCameraEntity(Camera cameraRequest) {
		...
		final GeoJsonPoint locationPoint = new GeoJsonPoint(Double.valueOf(cameraRequest.getLongitude()),
	...
	private List<Camera> cameraListResponse(List<CameraEntity> cameraList) {
		return cameraList.stream().map(object -> {
	...
	private List<Location> createLocationResponse(List<LocationEntity> locationList) {
		return locationList.stream().map(object -> {
	...
	private LocationEntity getLocationEntity(Location location) {
		...
		final GeoJsonPoint locationPoint = new GeoJsonPoint(Double.valueOf(location.getLongitude()),
